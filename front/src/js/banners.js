...
Banners.prototype.listenAddBannerEvent = function () {
    ...
    addBtn.click(function () {
        ...
        if(length >= 6){
            window.messageBox.showInfo('At most 6 figures！');
            return;
        }
                ...
                'confirmCallback': function () {
                    xfzajax.post({
                        ...
                        'success': function (result) {
                            if(result['code'] === 200){
                                bannerItem.remove();
                                window.messageBox.showSuccess('Delete successfully！');
                            }
                        }
                    });
                }
    ...
    saveBtn.click(function () {
        ...
        xfzajax.post({
            ...
            'success': function (result) {
                if(result['code'] === 200){
                    if(bannerId){
                        window.messageBox.showSuccess('Update successfully！');
                    }else{
                        bannerId = result['data']['banner_id'];
                        bannerItem.attr('data-banner-id',bannerId);
                        window.messageBox.showSuccess('Add successfully！');
                    }
